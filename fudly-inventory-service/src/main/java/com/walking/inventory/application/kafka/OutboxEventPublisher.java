...
import com.walking.inventory.domain.entity.outbox.OutboxStatus;
import com.walking.inventory.application.mapper.event.EventEnvelopeMapper;
import com.walking.inventory.domain.repository.OutboxEventRepository;
...
public class OutboxEventPublisher {
    ...
    public void publishPendingEvents() {
        ...
        for (OutboxEvent event : events) {
            try {
                ...
                kafkaTemplate.send(appProperties.getKafka().getTopics().getInventory(), event.getAggregateId().toString(), envelope)
    ...
    public void requeueExpiredEvents() {
        OffsetDateTime threshold = OffsetDateTime.now().minusMinutes(appProperties.getKafka().getOutbox().getLeaseTimeMinutes());
        Integer count = transactionTemplate.execute(status -> outboxEventRepository.requeueExpired(threshold));
    ...
    private List<OutboxEvent> claimBatch() {
        return transactionTemplate.execute(status -> {
            List<OutboxEvent> events = outboxEventRepository.findByPendingAndLock(appProperties.getKafka().getOutbox().getBatchSize());
